...
public:
  ...
  /// If ASMap is provided the function will attempt to convert language to
  /// target AS.
  static bool isAddressSpaceSupersetOf(LangAS A, LangAS B,
                                       const LangASMap *ASMap = nullptr,
                                       bool IsSYCLOrOpenCL = false) {
    if (ASMap) {
      // It would be better to perform the conversion in the other direction
      // (to language AS), as that would allow for overlapping of addresses,
      // but this would require API change. As a result only direct match with
      ...
      if (IsATargetAS ^ IsBTargetAS) {
        ...
        if (IsATargetAS)
          ...
          A = static_cast<LangAS>(
              ...
              static_cast<unsigned>(LangAS::FirstTargetAddressSpace));
        // When dealing with target AS only return true if A is equal to B or A
        // is generic and B is not constant (making sure that constant and
        // generic are set).
        if (IsSYCLOrOpenCL)
          return A == B ||
