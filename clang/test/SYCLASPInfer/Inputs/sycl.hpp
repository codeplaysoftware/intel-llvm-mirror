...
class property_list {
public:
  template <typename... propertiesTN> property_list(propertiesTN... props){};
...
template <typename T>
class [[__sycl_detail__::
            device_global]] [[__sycl_detail__::
                                  global_variable_allowed]] device_global {
public:
...
template <class _name, class _dataT> class host_pipe {
...
public:
  ...
  static _dataT read() { (void)__pipe; }
};
...
template <int dim> struct item {
  template <typename... T> item(T... args) {} // fake constructor
...
template <int dim> struct _ImplT {
  range<dim> AccessRange;
  range<dim> MemRange;
  id<dim> Offset;
};

template <typename T, typename access::target accessTarget> struct ASPTrait {};
template <typename T> struct ASPTrait<T, access::target::global_buffer> {
  using type = __attribute__((opencl_global)) T;
};
template <typename T> struct ASPTrait<T, access::target::local> {
  using type = __attribute__((opencl_local)) T;
};
...
public:
  using InnerType = typename ASPTrait<dataT, accessTarget>::type;
  ...
  dataT *get() const { return _Ptr; }
  dataT &operator[](int i) const { return _Ptr[i]; }

protected:
  void __init(InnerType *Ptr, range<dimensions> AccessRange,
  ...
  void __init_esimd(InnerType *Ptr) {}
  friend class stream;

  InnerType *_Ptr;
  range<dimensions> _AccessRange;
  range<dimensions> _MemRange;
...
template <typename dataT, int dimensions, access::mode accessmode>
class __attribute__((sycl_special_class)) __SYCL_TYPE(
    accessor) accessor<dataT, dimensions, accessmode, access::target::image,
...
#ifdef __SYCL_DEVICE_ONLY__
  void
  __init(typename opencl_image_type<dimensions, accessmode,
                                    access::target::image>::type ImageObj) {
    impl.MImageObj = ImageObj;
  }
#endif
};
...
__SYCL_TYPE(local_accessor) local_accessor
    : public accessor<dataT, dimensions, access::mode::read_write,
                      access::target::local> {
public:
  using InnerType =
      typename accessor<dataT, dimensions, access::mode::read_write,
                        access::target::local>::InnerType;
  void use(void) const {}
  template <typename... T> void use(T... args) {}
  template <typename... T> void use(T... args) const {}
  _ImplT<dimensions> impl;
...
protected:
  void __init(InnerType *Ptr, range<dimensions> AccessRange,
    ...
    accessor<dataT, dimensions, access::mode::read_write,
             access::target::local>::__init(Ptr, AccessRange, MemRange, Offset);
  }
};
...
template <typename Type> struct get_kernel_name_t<auto_name, Type> {
  using name = Type;
};
...
class __attribute__((sycl_special_class))
__SYCL_TYPE(annotated_arg) annotated_arg {
  T obj;
#ifdef __SYCL_DEVICE_ONLY__
  void __init(T _obj) {}
#endif
...
template <typename T, typename... Props>
class __attribute__((sycl_special_class))
__SYCL_TYPE(annotated_ptr) annotated_ptr {
  T *obj;
#ifdef __SYCL_DEVICE_ONLY__
  void __init(T *_obj) {}
#endif
...
class __SYCL_TYPE(kernel_handler) kernel_handler {
  void __init_specialization_constants_buffer(
      char *specialization_constants_buffer) {}
};
...
kernel_single_task(const KernelType &kernelFunc) { // #KernelSingleTask
  kernelFunc();
}
...
template <typename KernelName, typename KernelType>
ATTR_SYCL_KERNEL void
kernel_single_task_2017(KernelType kernelFunc) { // #KernelSingleTask2017
  kernelFunc();
}
...
template <typename T> const stream &operator<<(const stream &S, T &&) {
  return S;
}
...
template <int dimensions = 1, typename AllocatorT = int> class image {
public:
