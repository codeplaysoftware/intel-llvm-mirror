...
int main() {
  ...
  for (int i = 0; i < width; i++) {
    for (int j = 0; j < height; j++) {
      for (int k = 0; k < layers; k++) {
        expected[k + (layers) * (j + (height)*i)] =
            (k + (layers) * (j + (height)*i)) * 2;
        dataIn[k + (layers) * (j + (height)*i)] =
            k + (layers) * (j + (height)*i);
      }
    }
  }
  ...
  try {
    ...
    sycl::range copyExtent = {width / 2, height / 2, layers / 2};
    // The extent of data provided on the host (vector).
    sycl::range srcExtent = {width, height, layers};
    ...
    std::vector<sycl::range<3>> offsets{{0, 0, 0},
                                        ...
                                        {width / 2, 0, layers / 2},
                                        {0, height / 2, layers / 2},
                                        {width / 2, height / 2, layers / 2}};

    for (auto offset : offsets) {
      // Extension: Copy to image array subregion.
      q.ext_oneapi_copy(dataIn.data(), offset, srcExtent, imgMem0.get_handle(),
    ...
    q.submit([&](sycl::handler &cgh) {
      cgh.parallel_for<image_addition>(
          sycl::nd_range<3>{{width, height, layers}, {width, height, layers}},
          [=](sycl::nd_item<3> it) {
    ...
    for (auto offset : offsets) {
      q.ext_oneapi_copy(imgMem1.get_handle(), offset, desc, out.data(), offset,
