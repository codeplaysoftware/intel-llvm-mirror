...
#define FUNC __sycl_std::__invoke_tanh<T>
  ...
  for (size_t i = 0; i < N / 2; i++) {
    vec<T, 2> *in_vec = reinterpret_cast<vec<T, 2> *>(&x[i * 2]);
    vec<T, 2> *out_vec = reinterpret_cast<vec<T, 2> *>(&res[i * 2]);
    *out_vec = FUNC_VEC(*in_vec);
  }
  if (N % 2) {
    res[N - 1] = FUNC(x[N - 1]);
  }
...
#define FUNC __sycl_std::__invoke_exp2<half>
  ...
  for (size_t i = 0; i < N / 2; i++) {
    vec<half, 2> *in_vec = reinterpret_cast<vec<half, 2> *>(&x[i * 2]);
    vec<half, 2> *out_vec = reinterpret_cast<vec<half, 2> *>(&res[i * 2]);
    *out_vec = FUNC_VEC(*in_vec);
  }
  if (N % 2) {
    res[N - 1] = FUNC(x[N - 1]);
