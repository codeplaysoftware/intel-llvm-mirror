...
__SYCL_INLINE_NAMESPACE(cl) {
  ...
  // TODO: I think we can join the two below definitions into a single one  :
  // matrix_use::a || matrix_use::b
  template <matrix_layout Layout>
  struct joint_matrix<
  ...
  // TODO: for proposal could argue that two types of interfaces might be
  // eventually useful: the one below and a second which allows the user to use
  // preconverted bf16/tf32 types
  template <matrix::matrix_layout Layout, access::address_space Space>
  struct joint_matrix_load_impl<
...
#ifdef __SYCL_DEVICE_ONLY__
      uint32_t val[128];
      // this is just a temp solution to test if bf16 is working: should be made
      // simd etc if this approach retained: question is should it be users
      // responsibility to do the conversion or not?. Each thread is currently
      // resetting each element instead of it being assigned intelligently. ij
      // loop then stride!
      for (int i = 0; i < 8; i++)
        for (int j = 0; j < 16; j++) {
