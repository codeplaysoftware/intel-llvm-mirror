...
__SYCL_INLINE_NAMESPACE(cl) {
  ...
  using EnableIfIsPointer =
      cl::sycl::detail::enable_if_t<cl::sycl::detail::is_pointer<Ptr>::value,
                                    T>;
  ...
  void wait_for(Group g, eventTN... Events) {
    ...
    // __spirv_GroupWaitEvents ignores event_list and calls
  ...
  async_group_copy(sub_group, sub_group_mask mask, global_ptr<dataT> src,
                   local_ptr<dataT> dest, size_t numElements,
                   size_t srcStride) {
    using DestT = detail::ConvertToOpenCLType_t<decltype(dest)>;
    using SrcT = detail::ConvertToOpenCLType_t<decltype(src)>;
  ...
  async_group_copy(sub_group g, sub_group_mask mask, multi_ptr<T, SrcS> Src,
                   multi_ptr<T, DestS> Dest, size_t NumElements,
                   size_t Stride) {
    static_assert(sizeof(bool) == sizeof(uint8_t),
                  "Async copy to/from bool memory is not supported.");
    auto DestP =
  ...
  detail::enable_if_t<(detail::is_generic_group<Group>::value &&
                      ...
                      typename detail::remove_pointer<
                          Ptr>::type> reduce(Group g, Ptr first, Ptr last,
                                             BinaryOperation binary_op) {
  ...
  detail::enable_if_t<
      (detail::is_generic_group<Group>::value &&
       ...
       detail::is_native_op<T, BinaryOperation>::value),
      T> reduce(Group g, Ptr first, Ptr last, T init,
                BinaryOperation binary_op) {
