...
static VkCommandBuffer vk_transferCmdBuffer;
...
VkResult setupCommandBuffers() {
  ...
  if (vk_computeQueueFamilyIndex == vk_transferQueueFamilyIndex) {
    vk_transferCmdBuffer = vk_computeCmdBuffer;
  } else {
    VkCommandBufferAllocateInfo cbai = {};
    cbai.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
    ...
    cbai.commandBufferCount = 1;
    VK_CHECK_CALL_RET(
        vkAllocateCommandBuffers(vk_device, &cbai, &vk_transferCmdBuffer));
  }
...
int getMemoryOpaqueFD(VkDeviceMemory memory) {
  ...
  if (vk_getMemoryFdKHR != nullptr) {
    vk_getMemoryFdKHR(vk_device, &mgfi, &fd);
  }
  return fd;
}
